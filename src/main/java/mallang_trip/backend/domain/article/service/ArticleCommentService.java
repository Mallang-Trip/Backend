...
public class ArticleCommentService {
	...
	public Page<MyCommentResponse> getMyComments(Pageable pageable) {
		...
		Collections.sort(responses,
			Comparator.comparing(MyCommentResponse::getCreatedAt).reversed());
		// List -> PageImpl
		int start = (int) pageable.getOffset();
		int end = (start + pageable.getPageSize()) > responses.size() ? responses.size()
	...
	private List<MyCommentResponse> getMyComments(User user) {
		List<MyCommentResponse> comments = commentRepository.findByDeletedAndUser(false, user)
			.stream()
	...
	private List<MyCommentResponse> getMyReplies(User user) {
		List<MyCommentResponse> replies = replyRepository.findByDeletedAndUser(false, user).stream()
			.map(reply -> {
				...
				return MyCommentResponse.of(article, reply.getContent(), reply.getCreatedAt(),
					countCommentsAndReplies(article));
			})
			.collect(Collectors.toList());
		return replies;
	}
