...
public class IncomeAdminService {
     ...
     * (관리자) 수수료 비율을 변경합니다.
     *
     * @param partyCommissionPercent   변경할 파티 수수료 비율
     * @param penaltyCommissionPercent 변경할 위약금 수수료 비율
     */
    public void changeCommissionRate(double partyCommissionPercent,
        double penaltyCommissionPercent) {
        if (partyCommissionPercent < 0 || partyCommissionPercent > 100
            || penaltyCommissionPercent < 0 || penaltyCommissionPercent > 100) {
            throw new BaseException(Bad_Request);
        }
